    ...
    if d(text="不在打卡范围内").exists(timeout=2):
        ...
        while True:
            ...
            if MY().识别图片() != '不在打卡范围内':
                break
            continue
    elif d(text="上班·正常").exists(timeout=2) or d(text="上班自动打卡·正常").exists(timeout=2):
        print('\n已打上班卡')
        MY().截图()
        d.app_stop("com.tencent.mm")
        d.app_stop("com.tencent.wework")
...
def job1():
    ...
    for i in range(3):
        try:
            print('关闭usb选择')
            os.system('adb shell input tap 86 646')
            print(f'循环次数：{i + 1}')
            click("上班打卡")
            print('\n已打上班卡')
            countdown(5)
